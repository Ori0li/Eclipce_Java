...
//	("products": [{"name":"책상","price":600000,"category":4,quantity=55},{"name":"의자","price":350000,"category":4,quantity=55}])
//	쇼핑몰에서 판매하는 상품 자료 type = Variable Object(VO), Data Trnasfer Object(DTO)
//	JSON은 무엇인가요? -> JSON을 사용하는 이유!(통신 시 장점 1.문자열, 2.가볍다, 3.type 정의가능 4.data traveling이 용이)
	
// key:vlaue로 되어 있는 가장 인기있는 자료구조! JavaScriptObjectNotation : 객체 명세서(객체 설명서) => 리터럴 표기법으로 만들어짐 => 문자열 통신으로 사용
//	enum Cate // 각 필드가 고유값을 갖는 상수기 때문에 수를 사용하는 다른 상수와 섞어 쓰지 않는다.
	
//	enum Cate{식품,의류,가구,문구,침구}
	...
	class ProductDTO{
		// class의 전역에 선언된 변수 : 전역변수, 객체의 필드로 생성(속성)
		String name=null;
		int price=0;
		int category; // 0:식품 1:의류 2:가구 3:문구류 4:침구류
		int quantity=10;
		//생성자로 필드를 초기화:생성자의 return이 필요없는 이유 => 생성자는 new 연산자로 호출하면 객체를 반환하는데 이름이 반환하는 타입인 함수다.
	...
	public static void main(String[] args) {
		...
		}// 외부 반복문 검사식(함수형 언어가 지원하는 내부반복문 보다 복잡가 높다)
...
//		.forEach(System.out::println); // 최종연산
		...
		//카테고리가 가구인 것의 가격의 합을 구하시오(reduce)
		
		Optional<Integer> sumOpt = products.stream()
		.filter((p)->p.category==Cate.가구)
		.map(p->p.price*p.quantity) // 중간연산이 IntStream을 반환(기본형 스트림의 장점 1.null없음, 2.연산하는 함수를제공)
//		.mapToIn(p->p.price*p.quantity)
//		.sum();
		.reduce((p1,p2)->p1+p2);
//		.forEach(System.out::println);
		System.out.println("가구의 가격 총합: " +sumOpt.get());
		
		// 내부 반복문을 사용하면 stream 자료를 query처럼 제어할수 있다
		
		Integer i=null;
		//System.out.print(i+10); // 오류 null.intValue()
		i=30;
		System.out.println(i+10);
		
		
		// Optional : data가 null일수 있으니 처리하도록 명시하는 타입
		Optional<Integer> i_opt=Optional.empty(); // Optional 선언
		i_opt=Optional.ofNullable(99); // Optional data 참조하는 방법 // null이 아닌값만 참조
//		System.out.println(i_opt+20);
//		System.out.println(i_opt.get()+20);
		if(i_opt.isPresent()) {
			System.out.println(i_opt.get()+20);
		}else {
			System.out.println("isPresnet() 는 값이 있을때만 true를 반환, isEmpty()는 Null일떄만 True 변환");
		}
		//반환되는 데이터가 Null일 수 있음을 type으로 명시하는 것
//		i_opt.ifPresent((num)->{
//			System.out.println("값이 있을 때 실행되는 함수");
//		});
		
		i_opt.ifPresentOrElse((num)->{
			System.out.println("값이 있을 때");
		}, ()->{
			System.out.println("값이 없을 때(null)");
		});
...
// 파이썬, 자바스크립트 (인터프리터언어 : 소스코드를 바로 실행) : 컴파일시 발견되는 오류(편집기의 성능에 오류를 의존) <-> 런타임시 발견되는 오류
// 사용되지 않는 코드를 작성하는 것도 컴파일 오류
// 함수는 반환하는 data의 type을 명시한다.